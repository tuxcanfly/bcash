...
class NodeClient extends AsyncEmitter {
  ...
  init() {
    ...
    this.node.chain.on('disconnect', async (entry, block, view) => {
      if (!this.opened)
        return;
    ...
    this.node.chain.on('reset', async (tip) => {
      if (!this.opened)
        return;
  ...
  async getCoin(hash, index) {
    return this.node.chain.getCoin(hash, index);
  }
  ...
  async getBlockView(block) {
    return this.node.chain.getBlockView(block);
  }
  ...
  async getCoinView(tx) {
    return this.node.chain.getCoinView(tx);
  }
