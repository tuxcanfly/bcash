...
class RPC extends RPCBase {
  ...
  async _createTemplate(maxVersion, coinbase, rules) {
    ...
    const json = {
      ...
      sigoplimit: consensus.maxBlockSigops(consensus.MAX_FORK_BLOCK_SIZE),
      sizelimit: consensus.MAX_FORK_BLOCK_SIZE,
  ...
  async getRawTransaction(args, help) {
    ...
    const plugin = this.node.require('indexer');

    if (!plugin)
      throw new RPCError(errs.MISC_ERROR, 'Transaction not found.');

    const meta = await plugin.indexer.getMeta(hash);
