...
class BlockTemplate {
  ...
  createCoinbase() {
    ...
    assert(cb.getSize() >= consensus.MIN_TX_SIZE,
      'Coinbase transaction is too small!');
  ...
  addTX(tx, view) {
    ...
    const size = item.tx.getSize();
    const sigops = item.sigops;

    if (!tx.isFinal(this.height, this.locktime))
      return false;

    if (this.size + size > consensus.MAX_FORK_BLOCK_SIZE)
      return false;

    if (this.sigops + sigops > consensus.maxBlockSigops(this.size + size))
      return false;
...
/**
 * Sort transaction items by txid
 * @param {BlockEntry} a
 * @param {BlockEntry} b
 * @return {Number}
 */

function sortByTXID(a, b) {
