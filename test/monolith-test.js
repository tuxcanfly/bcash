...
describe('Monolith', function() {
  it('should fail monolith when flag disabled', async () => {
    ...
    let err;
    try {
      script.execute(stack, common.flags.STANDARD_VERIFY_FLAGS);
    } catch (e) {
      err = e;
    }
    assert.typeOf(err, 'error');
  ...
  for (const test of minimalTestCases) {
    it(`should match minimal encoding ${test[0].toString('hex')}`, async () => {
  ...
  for (const test of bitwiseTestCases) {
    it(`should match bitwise ops - ${test[0]}`, async () => {
  ...
  for (const test of stringTestCases) {
    it(`should match string ops - ${test[0]} + ${test[1]}`, async () => {
  ...
  for (const test of typeConversionTestCase) {
    it(`should match type conversion ops - ${test[0]}.${test[1]}`, async () => {
  ...
  for (const test of arithmeticTestCases) {
    it(`should match arithmetic ops - ${test[0]} x ${test[1]}`, async () => {
      const a = Buffer.from(test[0], 'hex');
      const b = Buffer.from(test[1], 'hex');
